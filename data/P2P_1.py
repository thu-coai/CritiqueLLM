...
base_prompt = "你是一个擅长评价文本质量的助手。\n请你以公正的评判者的身份，比较两个AI助手对于用户提问的回答的质量优劣。" \
            "由于您评估的回答类型是{category}，因此你需要从下面的几个维度对两个回答进行细致的比较和分析:\n{dimensions}" \
            "我们会给你提供用户的提问，高质量的参考答案，需要你比较的两个AI助手的答案，以及两个答案各自的质量评价分析。" \
            "当你开始你的评估时，你需要遵守以下的流程：\n" \
            "1. 结合参考答案、两个AI助手的答案以及其质量评价分析，根据上述指定的维度对他们的答案进行细致的比较，给出详细的比较分析文本。" \
            "比较分析文本要求覆盖两个答案的质量评价分析中可用于比较的所有重要细节，并包含对答案中具体内容的分析。\n" \
            "2. 结合参考答案和每个维度的比较分析，从两个AI助手的答案中选出综合质量更高的那个，或者判定他们质量相当，并给出详尽的选择理由。\n" \
            ...
            "[助手2的答案开始]\n{response_2}\n[助手2的答案结束]\n" \
            "[助手2的答案质量评价分析开始]\n{pointwise_reference_based_judgement_2}\n[助手2的答案质量评价分析结束]\n"
    ...
    def gpt(self, payload):
        while True:
            try:
                ...
                data['pointwise_reference_based_to_pairwise_prompt'] = payload['messages'][0]['content']
...
if __name__ == "__main__":
    ...
    for i, d in enumerate(data):
        ...
        prompt = base_prompt.format(category=d['category'], 
                                    ...
                                    pointwise_reference_based_judgement_1=d['pointwise_reference_based_judgement_1'],
                                    pointwise_reference_based_judgement_2=d['pointwise_reference_based_judgement_2']
                                    )
        payload = {
    ...
    outs = []
    for i, d in enumerate(data):
        d['pointwise_reference_based_to_pairwise_prompt'] = results[i]['pointwise_reference_based_to_pairwise_prompt']
        d['pointwise_reference_based_to_pairwise_judgement'] = results[i]['pointwise_reference_based_to_pairwise_judgement']
        outs.append(d)
